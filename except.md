...
在第 2 情况中，如果上移操作导致其双亲节点中元素个数大于 M，则应继续分裂其双亲节点。

![](Algorithm/img/bpt5.png)

插入元素 40 ，按照第 2 种情况将节点分裂，并将元素 37 上移到父节点，发现父节点`[15,37,44,59]`包含的元素的个数大于 M ，所以将节点`[15,37,44,59]`分裂为两个节点`[15,37]`和节点`[44,59]`，并将元素 37 上移到父节点`[59,97]`中. 父节点包含元素个数没有超过 M ，插入结束。

### 情况4

若插入的元素比当前结点中的最大值还大，破坏了B+树中从根结点到当前结点的所有索引值，此时需要及时修正后，再做其他操作。

![](Algorithm/img/bpt6.png)

插入元素 100，由于其值比最大值 97 还大，插入之后，从根结点到该结点经过的所有结点中的所有值都要由 97 改为 100。改完之后再做分裂操作。
