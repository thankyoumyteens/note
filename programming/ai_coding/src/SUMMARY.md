# Summary

- [大模型服务提供商](./provider/provider.md)
  - [EdenAI](./provider/eden_ai.md)
  - [OpenRouter](./provider/open_router.md)
  - [ShuttleAI](./provider/shuttle_ai.md)

- [安装 AI Coding 工具](./software/software.md)
  - [安装 Gemini CLI](./software/gemini_cli.md)
  - [安装 Codex CLI](./software/codex_cli.md)
  - [安装 Claude Code](./software/claude_code.md)
  - [安装 OpenCode](./software/open_code.md)
  - [安装 Goose](./software/goose.md)

- [AI 辅助开发](./work_with_ai/work_with_ai.md)
  - [把“我要一个 Java 项目”变成清晰任务](./work_with_ai/lesson1/lesson1.md)
    - [本课任务结构](./work_with_ai/lesson1/structure.md)
    - [推荐 Prompt](./work_with_ai/lesson1/prompt.md)
    - [你要检查 AI 的输出](./work_with_ai/lesson1/tips.md)
    - [如果 AI 过度设计，怎么纠正](./work_with_ai/lesson1/guide.md)
    - [如果 AI 创建项目失败，怎么让它修](./work_with_ai/lesson1/bug_fix.md)
  - [第 2 课：建立项目规则文件 AGENTS.md / CLAUDE.md](./work_with_ai/lesson2/lesson2.md)
    - [AGENTS.md 和 CLAUDE.md 的区别](./work_with_ai/lesson2/compare.md)
    - [本课推荐 Prompt](./work_with_ai/lesson2/prompt.md)
    - [重点观察 AI 有没有犯这些错误](./work_with_ai/lesson2/guide.md)
  - [第 3 课：初始化 Git 与基线提交](./work_with_ai/lesson3/lesson3.md)
    - [本课推荐 Prompt](./work_with_ai/lesson3/prompt.md)
    - [Claude Code 完成后你自己执行的命令](./work_with_ai/lesson3/cmd.md)
    - [如果 Claude Code 误改了代码，怎么纠正](./work_with_ai/lesson3/guide1.md)
  - [第 3A 课：项目启动流程工具化](./work_with_ai/lesson3a/lesson3a.md)
    - [本课推荐 Prompt](./work_with_ai/lesson3a/prompt.md)
    - [你要重点检查什么](./work_with_ai/lesson3a/check.md)
  - [第 4 课：从模糊需求到清晰任务](./work_with_ai/lesson4/lesson4.md)
    - [本课推荐 Prompt](./work_with_ai/lesson4/prompt.md)
    - [你应该期待的输出](./work_with_ai/lesson4/check.md)
    - [不符合预期怎么纠正](./work_with_ai/lesson4/guide.md)
  - [第 5 课：输出格式控制](./work_with_ai/lesson5/lesson5.md)
    - [本课推荐 Prompt](./work_with_ai/lesson5/prompt.md)
    - [不符合预期怎么纠正](./work_with_ai/lesson5/guide.md)
  - [第 5A 课：Prompt 模板工具化：Commands / Skills](./work_with_ai/lesson5a/lesson5a.md)
    - [Commands / Skills 分别是什么](./work_with_ai/lesson5a/concept.md)
    - [本课要设计的 3 个模板](./work_with_ai/lesson5a/templates.md)
    - [本课推荐 Prompt](./work_with_ai/lesson5a/prompt.md)
    - [你要重点检查什么](./work_with_ai/lesson5a/check.md)
  - [第 6 课：先计划，后执行](./work_with_ai/lesson6/lesson6.md)
    - [为什么必须先计划](./work_with_ai/lesson6/why.md)
    - [本课推荐 Prompt](./work_with_ai/lesson6/prompt.md)
    - [本课重点检查](./work_with_ai/lesson6/check.md)
    - [不符合预期怎么纠正](./work_with_ai/lesson6/guide.md)
  - [第 7 课：小步实现文档保存功能](./work_with_ai/lesson7/lesson7.md)
    - [本课推荐 Prompt](./work_with_ai/lesson7/step1.md)
  - [第 7A 课：Plan-Then-Act 工具化](./work_with_ai/lesson7a/lesson7a.md)
    - [本课推荐 Prompt](./work_with_ai/lesson7a/prompt.md)
    - [添加到 WORKFLOW.md 后怎么用？](./work_with_ai/lesson7a/use.md)
  - [第 8 课：轻量 Spec 结构](./work_with_ai/lesson8/lesson8.md)
    - [为什么需要 Spec Workflow](./work_with_ai/lesson8/why_spec.md)
    - [本课推荐 Prompt](./work_with_ai/lesson8/prompt.md)
    - [你要检查 Spec 是否合格](./work_with_ai/lesson8/requirements.md)
  - [第 9 课：Spec 驱动实现文档查询功能](./work_with_ai/lesson9/lesson9.md)
    - [本课推荐 Prompt](./work_with_ai/lesson9/prompt.md)
  - [第 9A 课：Spec Workflow 工具化：spec-workflow-mcp](./work_with_ai/lesson9a/lesson9a.md)
    - [前置环境](./work_with_ai/lesson9a/env.md)
    - [先在项目目录测试能否启动](./work_with_ai/lesson9a/test.md)
    - [启动 Dashboard](./work_with_ai/lesson9a/dashboard.md)
    - [配置到 Claude Code](./work_with_ai/lesson9a/cc.md)
    - [配置到 Codex](./work_with_ai/lesson9a/codex.md)
    - [VS Code Extension](./work_with_ai/lesson9a/vscode.md)
    - [它可能生成什么目录](./work_with_ai/lesson9a/dirs.md)
  - [第 9B 课：用 spec-workflow-mcp 做一个小功能](./work_with_ai/lesson9b/lesson9b.md)
    - [本课开始前检查](./work_with_ai/lesson9b/check.md)
    - [本课推荐 Prompt](./work_with_ai/lesson9b/prompt.md)
  - [第 10 课：先写测试，不写实现](./work_with_ai/lesson10/lesson10.md)
    - [什么是 TDD](./work_with_ai/lesson10/tdd.md)
    - [本课推荐 Prompt](./work_with_ai/lesson10/prompt.md)
  - [第 11 课：最小实现让测试通过](./work_with_ai/lesson11/lesson11.md)
    - [本课推荐 Prompt](./work_with_ai/lesson11/prompt.md)
  - [第 11A 课：在测试保护下小步重构](./work_with_ai/lesson11a/lesson11a.md)
    - [本课推荐 Prompt](./work_with_ai/lesson11a/prompt.md)
  - [第 11B 课：测试工作流工具化](./work_with_ai/lesson11b/lesson11b.md)
    - [本课推荐 Prompt](./work_with_ai/lesson11b/prompt.md)
    - [Skill 文件建议结构](./work_with_ai/lesson11b/skill_file.md)
    - [使用 Skill](./work_with_ai/lesson11b/use_skill.md)
      - [把 Skill 安装到 Codex](./work_with_ai/lesson11b/use_skill_codex.md)
      - [把 Skill 安装到 Claude Code](./work_with_ai/lesson11b/use_skill_cc.md)
      - [在 AGENTS.md / CLAUDE.md 里加“自动使用规则”](./work_with_ai/lesson11b/use_skill2.md)
      - [显式告诉 AI 使用某个 Skill](./work_with_ai/lesson11b/use_skill1.md)
  - [第 11C 课：使用 spec-workflow-mcp + TDD Skill 的组合实践](./work_with_ai/lesson11c/lesson11c.md)
    - [创建组合 Skill](./work_with_ai/lesson11c/prompt1.md)
    - [使用组合 Skill 实战小功能](./work_with_ai/lesson11c/prompt.md)
    - [因不可抗力中断后让 AI 继续执行任务](./work_with_ai/lesson11c/prompt2.md)
  - [第 12 课：Hooks 自动化：测试、格式化、提醒](./work_with_ai/lesson12/lesson12.md)
    - [本课推荐 Prompt](./work_with_ai/lesson12/prompt.md)

- [主 Agent / Sub Agent 分工工作流](./sub_agent/sub_agent.md)
  - [项目准备](./sub_agent/demo_project.md)
  - [阶段 0：项目级上下文校准](./sub_agent/stage0/stage0.md)
    - [创建文档](./sub_agent/stage0/step1.md)
    - [验收文档](./sub_agent/stage0/step2.md)
  - [阶段 1：需求探索与反复讨论](./sub_agent/stage1/stage1.md)
    - [要实现的需求](./sub_agent/stage1/new_function.md)
    - [讨论需求](./sub_agent/stage1/discuss.md)
  - [阶段 2：需求定稿](./sub_agent/stage2/stage2.md)

- [SKILL](./skills/skills.md)
  - [基于 SKILL.md 的 Agent Skills](./skills/md_skill.md)
  - [基于 MCP 接入自定义工具](./skills/mcp_skill.md)
  - [常用 SKILL](./skills/examples/examples.md)

- [常用插件](./plugins/plugins.md)
- [spec-workflow-mcp](./plugins/spec_workflow/spec_workflow.md)
  - [安装 spec-workflow-mcp](./plugins/spec_workflow/spec_workflow_mcp.md)
    - [Gemini CLI 配置 spec-workflow-mcp](./plugins/spec_workflow/spec_workflow_mcp_gemini.md)
    - [Codex CLI 配置 spec-workflow-mcp](./plugins/spec_workflow/spec_workflow_mcp_codex.md)
    - [Claude Code 配置 spec-workflow-mcp](./plugins/spec_workflow/spec_workflow_mcp_cc.md)
    - [Goose 配置 spec-workflow-mcp](./plugins/spec_workflow/spec_workflow_mcp_goose.md)

- [SOP](./sop/sop.md)
  - [iOS 端静默断网抢救指南(FastAPI + Nginx 架构专属)](./sop/ios_bug.md)
