...
## Aider 使用 spec-workflow-mcp

## Claude Code 使用 spec-workflow-mcp

通过配置 `.mcp.json` 文件实现。`.mcp.json` 文件可以写到两个地方：

1. 用户的家目录：提供通用的工具和环境变量
2. 特定项目的根目录：提供仅对当前项目生效的特定工具

当你运行 Claude Code 时，底层的 MCP 客户端通常会执行一套“合并逻辑”：它会先读取 家目录的全局配置 (`~/.mcp.json`) 作为底座，然后再读取 当前项目的局部配置 (`./.mcp.json`)。家目录全局配置的优先级高于当前目录。
