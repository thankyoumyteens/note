...
CSet 是 G1 的回收集, 代表每次垃圾回收要处理的的一系列目标 Region。在任意一次 GC 中, CSet 所有 Region 都会被释放, 内部存活的对象都会被转移到的空闲 Region 中。因此无论是 Young GC, 还是 Mixed GC, 工作的机制都是一致的。Young GC 的 CSet 是所有的新生代 Region, 而 Mixed GC 会通过启发式算法, 在老年代 Region 中, 筛选出回收收益最高的 Region 添加到 CSet 中。
...
并发标记主要是为 Mixed GC 提供标记服务的, 并不是一次 GC 过程的一个必须环节。
