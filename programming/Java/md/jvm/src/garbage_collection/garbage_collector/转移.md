...
3. 将对象a引用的所有位于回收集内的对象都添加到转移队列中。转移队列用来临时保存待转移对象的引用方。假设a'.field1引用了对象b，而且b所在的Region在回收集中。因为a'是存活对象，所以a'引用的对象b也是存活对象。这样一来，对象b就成了回收集中的待转移对象，它的引用方a'.field1会被添加到转移队列中。之所以往转移队列中添加a'.field1而不是b，是因为我们必须要在转移完b之后将新的地址写入到a'.field1中
![](../../img/g1bm13.png)

4. 针对对象a引用的位于回收集外的对象，更新RSet。a'.field2引用了对象c，而c所在的Region不在回收集中。c所在Region的RSet中虽然记录了a所在卡页对应的卡片索引，但是a被转移到了a'，所以有必要更新RSet。a'所在卡页对应的卡片索引被添加到了c所在Region的RSet中
![](../../img/g1bm14.png)
...
ptr evacuate_obj(ref) {
    ...
    for (child in to.children()) {
        // 检查子对象是否在回收集内
        if (is_into_collection_set(*child)) {
            // 如果在回收集内，则将子对象添加到转移队列evacuate_queue中
            // 对应上面第3步
            enqueue(evacuate_queue, child);
        } else {
            // 如果不在回收集内，则将子对象的引用方添加到子对象的RSet中
            // 对应上面第4步
...
void add_reference(from, to) {
    ...
    // 还会检查from是否在回收集之外
    // 如果from在回收集中，那么它要么已经转移完成，要么马上就要被转移，所以都可以忽略掉
    if (to_region != NULL
        && from_region != NULL
...
完成根转移之后，那些被转移队列引用的对象将会依次转移。直到转移队列被清空，转移就完成了。至此，回收集合内的所有存活对象都被成功转移了。
