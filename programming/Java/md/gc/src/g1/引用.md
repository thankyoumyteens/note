...
当内存不足，会触发 GC，如果 GC 后，内存还是不足，就会清理软引用。软引用适合用作缓存，当内存足够，可以正常的拿到缓存，当内存不够，就会先清理缓存，不至于马上抛出 OOM。
...
弱引用的特点是不管内存是否足够，只要发生 GC，都会被回收。弱引用在很多地方都有用到，比如 ThreadLocal、WeakHashMap。
