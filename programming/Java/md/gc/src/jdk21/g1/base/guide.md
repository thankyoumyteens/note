...
这是 G1 与其他回收器之间主要差异的总结：

- Parallel GC 只能整体地对老年代进行整理和回收空间。而 G1 则将这项工作增量地分布在多个更短的回收过程中。这显著缩短了停顿时间, 但可能会以吞吐量为代价
- G1 部分老年代的空间回收是并发进行的
- G1 可能会表现出比上述回收器更高的开销, 由于其并发性质, 这会影响吞吐量
- ZGC 旨在提供显著更小的停顿时间, 但会进一步以吞吐量为代价

由于其工作方式, G1 具有一些机制来提高垃圾回收的效率：

- G1 可以在任何回收过程中回收老年代中完全空闲的、较大的区域。这可以避免许多不必要的垃圾回收, 无需太多努力就能释放大量空间
- G1 可以选择性地尝试并发地去重 Java 堆中的重复字符串

回收老年代中空闲的大型对象始终是启用的。可以使用 `-XX:-G1EagerReclaimHumongousObjects` 选项来禁用此功能。字符串去重默认是禁用的。可以使用 `-XX:+G1EnableStringDeduplication` 选项来启用它。
