...
本节描述了本主题中介绍的命令行选项的默认值及一些附加信息。

| 配置项和默认值                                           | 说明                                                                                                                                                                                                                                                                                                                                                                                |
| -------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| -XX:+ReduceInitialCardMarks                              | 这会将初始对象分配的并发记忆集更新(细化, refinement)工作一起批量处理                                                                                                                                                                                                                                                                                                              |
| -XX:+ParallelRefProcEnabled -XX:ReferencesPerThread=1000 | `-XX:ReferencesPerThread` 决定了并行化的程度：对于每 N 个引用对象，将有一个线程参与到引用处理的子阶段中，但这一并行化是受 `-XX:ParallelGCThreads` 限制的。如果该值设置为 0，则表示将始终使用 `-XX:ParallelGCThreads` 所指定的最大线程数来进行引用处理。这意味着当值为 0 时，系统会尽可能多地利用可用线程来加速引用处理过程。这决定了是否应由多个线程并行处理 `java.lang.Ref.*` 实例 |
| -XX:G1RSetUpdatingPauseTimePercent=10                    | 此选项用于控制并发记忆集更新(细化, refinement)工作。细化工作试图被并发调度，以确保在“更新 RS”(Update RS)阶段的垃圾回收暂停期间，处理剩余工作的时间不超过最大停顿时间目标的 `-XX:G1RSetUpdatingPauseTimePercent` 百分比。这意味着通过调整该参数，可以影响在垃圾回收暂停期间用于处理记忆集更新的时间比例，从而平衡并发处理与停顿时间之间的关系                                    |
| -XX:G1SummarizeRSetStatsPeriod=0                         | 这指定了 G1 在每多少次垃圾回收周期(GCs)中生成一次记忆集摘要报告的周期。将此值设置为零可禁用该功能。由于生成记忆集摘要报告是一个成本较高的操作，因此仅在必要时使用，并且建议设置一个相对较高的值。若要输出记忆集的相关信息，可以使用 `gc+remset=trace` 选项                                                                                                                        |
| -XX:GCTimeRatio=12                                       | 这是用于计算垃圾回收时间与应用程序运行时间之间目标比例的除数。具体的计算公式为 1 / (1 + GCTimeRatio)，以确定在需要增加堆大小之前，可以花费在垃圾回收上的目标时间比例。默认值会使大约 8% 的时间被用于垃圾回收。这意味着，默认情况下，G1 会尝试将垃圾回收的时间控制在总运行时间的大约 8%，以确保应用程序有足够的运行时间                                                              |
| -XX:G1PeriodicGCInterval=0                               | 检查 G1 是否应该触发周期性垃圾回收的间隔时间(以毫秒为单位)。设置为零以禁用此功能                                                                                                                                                                                                                                                                                                  |
| -XX:+G1PeriodicGCInvokesConcurrent                       | 如果设置此选项，周期性的垃圾回收将触发并发标记或继续现有的回收周期；否则，将触发一次 Full GC                                                                                                                                                                                                                                                                                        |
| -XX:G1PeriodicGCSystemLoadThreshold=0.0                  | 这是通过主机的 `getloadavg()` 系统调用返回的当前系统负载阈值，用于决定是否应触发周期性垃圾回收。如果当前系统负载高于设定的阈值，则不会触发周期性垃圾回收。设置为零表示禁用该阈值检查，即不考虑系统负载情况，按照其他规则决定是否进行周期性垃圾回收。这有助于避免在系统负载已经很高的情况下增加额外负担                                                                              |
