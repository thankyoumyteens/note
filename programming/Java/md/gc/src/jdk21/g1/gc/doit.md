...
1. 如果发起 GC 的线程要求的是执行 GC 并分配内存(word_size 大于 0), 那么 JVM 会先尝试分配内存, 如果分配成功就可以省去一次 GC
2. 如果分配内存失败, 或者发起 GC 的线程要求的只是执行 GC, 那么 JVM 会开始执行增量的垃圾回收(Young GC 或 Mixed GC)
3. GC 执行成功后, 如果需要分配内存(word_size 大于 0), JVM 会再次尝试分配。否则会根据需要执行 Full GC
...
void VM_G1CollectForAllocation::doit() {
    ...
    if (_word_size > 0) {
        // _word_size大于0表示发起GC的线程同时请求了分配内存, 先尝试分配内存
        _result = g1h->attempt_allocation_at_safepoint(_word_size,
                                                       false /* expect_null_cur_alloc_region */);
        if (_result != nullptr) {
            // 如果内存分配成功, 虽然还没做垃圾回收, 也会认为垃圾回收成功了
            _gc_succeeded = true;
            return;
        }
    }
...
bool is_maximal_no_gc() const override {
    // 没有空闲的分区
    // _hrm.available(): 未提交的分区个数
    return _hrm.available() == 0;
}

// 获取空闲分区列表, num_free_regions()返回空, 表示堆无法再扩展
...
HeapWord *G1CollectedHeap::attempt_allocation_at_safepoint(size_t word_size,
    ...
    if (!is_humongous(word_size)) {
        // 复用加锁分后配的代码(此时在安全点中, 不会有其它Java线程在分配内存, 无需获取锁)
