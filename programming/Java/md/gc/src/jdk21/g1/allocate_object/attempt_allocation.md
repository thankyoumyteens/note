# CAS 分配

JVM 使用变量 `_alloc_region` 指向当前用来分配内存(不一定是对象的内存)的分区, 但内存不一定总是分配到 `_alloc_region` 指向的分区中。

在堆中分配内存时, 如果当前的分区内存不够分配, JVM 会申请一个新的分区, 这样原来的分区在被回收之前就不会再用来分配, 会造成空间的浪费, 所以如果原来的分区空间还足够大(大于 MinTLABSize, 即至少还能在上面分配一个 TLAB), JVM 就会保留这个分区, 用来分配小一点的内存。这个分区就称为 retained alloc region。

JVM 使用变量 `_retained_alloc_region` 指向 retained alloc region。一个分区不再使用时, 就会和现有的 retained alloc region 比较, 如果它的容量更大, 就会取代旧的 retained alloc region, 使 `_retained_alloc_region` 指向自己。

在堆中分配内存时, JVM 会先尝试在 retained alloc region 中分配, 当 retained alloc region 不够分配时, 才会在 `_alloc_region` 指向的分区中分配。

```cpp
// --- src/hotspot/share/gc/g1/g1Allocator.inline.hpp --- //

inline HeapWord *G1Allocator::attempt_allocation(size_t min_word_size,
                                                 size_t desired_word_size,
                                                 size_t *actual_word_size) {
    // 如果开启了NUMA, 内存就会被分成若干个节点, JVM会优先在相同的节点中分配内存,
    // 后面的mutator_alloc_region函数会根据这个索引找到合适的分区用来分配内存
    // 如果没有开启NUMA的话, 会返回0
    uint node_index = current_node_index();

    // 先试试retained alloc region够不够分配
    HeapWord *result = mutator_alloc_region(node_index)->attempt_retained_allocation(min_word_size, desired_word_size,
                                                                                     actual_word_size);
    if (result != nullptr) {
        return result;
    }

    // retained alloc region不够分配, 在当前分区中分配
    return mutator_alloc_region(node_index)->attempt_allocation(min_word_size, desired_word_size, actual_word_size);
}
```

## 在 retained alloc region 中分配

调用 par_allocate 分配内存。

```cpp
// --- src/hotspot/share/gc/g1/g1AllocRegion.inline.hpp --- //

inline HeapWord *MutatorAllocRegion::attempt_retained_allocation(size_t min_word_size,
                                                                 size_t desired_word_size,
                                                                 size_t *actual_word_size) {
    if (_retained_alloc_region != nullptr) {
        HeapWord *result = par_allocate(_retained_alloc_region, min_word_size, desired_word_size, actual_word_size);
        if (result != nullptr) {
            trace("alloc retained", min_word_size, desired_word_size, *actual_word_size, result);
            return result;
        }
    }
    return nullptr;
}
```

## 在当前 region 中分配

也是调用 par_allocate 分配内存。

```cpp
// --- src/hotspot/share/gc/g1/g1AllocRegion.inline.hpp --- //

inline HeapWord* G1AllocRegion::attempt_allocation(size_t min_word_size,
                                                   size_t desired_word_size,
                                                   size_t* actual_word_size) {
  HeapRegion* alloc_region = _alloc_region;
  assert_alloc_region(alloc_region != nullptr, "not initialized properly");
  HeapWord* result = par_allocate(alloc_region, min_word_size, desired_word_size, actual_word_size);
  if (result != nullptr) {
    trace("alloc", min_word_size, desired_word_size, *actual_word_size, result);
    return result;
  }
  trace("alloc failed", min_word_size, desired_word_size);
  return nullptr;
}

```

## 使用 CAS 分配内存

和 tlab 类似, bottom 指针和 end 指针分别指向了一个分区的起始位置和结束位置, top 指针指向已分配内存的边界, 在分区中分配内存只需要向 end 方向移动 top 指针就可以了, 区别是分区中需要考虑线程同步问题。

![](../../../img/region_pointer.jpg)

```cpp
// --- src/hotspot/share/gc/g1/g1AllocRegion.inline.hpp --- //

inline HeapWord *G1AllocRegion::par_allocate(HeapRegion *alloc_region,
                                             size_t min_word_size,
                                             size_t desired_word_size,
                                             size_t *actual_word_size) {
    assert(alloc_region != nullptr, "pre-condition");
    assert(!alloc_region->is_empty(), "pre-condition");

    return alloc_region->par_allocate(min_word_size, desired_word_size, actual_word_size);
}

// --- src/hotspot/share/gc/g1/heapRegion.inline.hpp --- //

inline HeapWord *HeapRegion::par_allocate(size_t min_word_size,
                                          size_t desired_word_size,
                                          size_t *actual_word_size) {
    return par_allocate_impl(min_word_size, desired_word_size, actual_word_size);
}

// 使用CAS分配内存
// desired_word_size 期望分配的内存大小
// min_word_size 至少要分配的内存大小
// actual_size 实际分配的内存大小
inline HeapWord *HeapRegion::par_allocate_impl(size_t min_word_size,
                                               size_t desired_word_size,
                                               size_t *actual_size) {
    do {
        // top指向这个分区的剩余空间的起始位置
        HeapWord *obj = top();
        // 计算这个分区还有多大的空闲内存
        size_t available = pointer_delta(end(), obj);
        // 最大能分配多大的内存
        size_t want_to_allocate = MIN2(available, desired_word_size);
        // 判断够不够分配
        if (want_to_allocate >= min_word_size) {
            // 堆中也是使用指针碰撞分配内存, 分配内存时只需要移动top指针即可
            HeapWord *new_top = obj + want_to_allocate;
            // CAS操作
            // _top: 要修改的地址
            // obj: 当前值
            // new_top: 新值
            HeapWord *result = Atomic::cmpxchg(&_top, obj, new_top);
            // CAS操作如果成功会返回旧值, 否则返回新值
            if (result == obj) {
                // CAS操作成功
                assert(is_object_aligned(obj) && is_object_aligned(new_top), "checking alignment");
                *actual_size = want_to_allocate;
                return obj;
            }
        } else {
            // 分区内存不够分配
            return nullptr;
        }
        // CAS操作失败, 等待下次循环重试
    } while (true);
}
```
