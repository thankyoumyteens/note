...
| 偏移量(offset) | 长度(字节) | 描述                                       |
| -------------- | ---------- | ------------------------------------------ |
| 0              | 4          | Local File Header 签名，固定值: 0x504B0304 |
| 4              | 2          | 提取本文件需要的最低版本号                 |
| 6              | 2          | 标志位                                     |
| 8              | 2          | 压缩方法                                   |
| 10             | 2          | 文件最后修改时间                           |
...
| 偏移量(offset) | 长度(字节) | 描述                                                       |
| -------------- | ---------- | ---------------------------------------------------------- |
| 0              | 4          | ZIP64 EOCD 的签名，固定值: 0x504B0606                      |
...
| 偏移量(offset) | 长度(字节) | 描述                                          |
| -------------- | ---------- | --------------------------------------------- |
| 0              | 4          | ZIP64 EOCD Locator 的签名，固定值: 0x504B0607 |
...
1. EOCD(0x504B0506)
2. ZIP64 EOCD Locator(0x504B0607)
3. ZIP64 EOCD Record(0x504B0606)
4. …（前面是 Central Directory 和 Local Headers）

## ZIP64 文件的解析流程

1. 从文件尾向前扫描找到传统 EOCD(签名 0x504B0506)
2. 再往前尝试找 ZIP64 EOCD Locator(签名 0x504B0607)
3. 如果找到 Locator：
