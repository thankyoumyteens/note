...
- PAOPAGATION_REQUIRE_NEW: 若当前没有事务，则新建一个事务。若当前存在事务，则新建一个事务，当前事务会被挂起，直到新事务执行完毕
...
- 前置通知@Before: 在目标方法执行之前执行的通知, 一般用来进行一些预处理操作，如权限验证、日志记录等
- 后置通知@After: 无论目标方法是否正常执行结束，都会在目标方法执行之后执行的通知。后置通知通常用于资源清理等操作
- 返回通知@AfterReturning: 在目标方法正常执行并返回结果后执行的通知。返回通知可以访问目标方法的返回值，可用于对返回结果进行一些处理或记录
- 异常通知@AfterThrowing: 在目标方法抛出异常时执行的通知。异常通知可以捕获目标方法抛出的异常，并进行相应的处理，如记录异常信息、进行异常转换等
- 环绕通知@Around: 在目标方法执行前后都进行增强处理。环绕通知可以控制目标方法是否执行、何时执行，还可以修改目标方法的输入参数和返回值
