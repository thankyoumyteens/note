# Summary

- [Java](./java/java.md)
  - [SuppressWarnings](./java/SuppressWarnings.md)
  - [BigDecimal](./java/BigDecimal.md)
  - [集合](./java/collection.md)
    - [List去重](./java/list_distinct.md)
  - [泛型](./java/parameterized_type.md)
    - [获取字段的参数化类型](./java/field_parameterized_type.md)
    - [获取方法的参数化类型](./java/method_parameterized_type.md)
    - [通过TypeReference获取泛型信息](./java/type_reference.md)
  - [动态代理](./proxy/proxy.md)
    - [JDK动态代理](./proxy/jdk_proxy.md)
    - [CGLIB动态代理](./proxy/cglib_proxy.md)

- [正则表达式](./java/regex/regex.md)
  - [匹配](./java/regex/match.md)
  - [查找](./java/regex/search.md)
  - [替换](./java/regex/replace.md)
  - [分组替换](./java/regex/group_replace.md)

- [路径和文件](./java/path_and_file.md)
  - [路径操作](./java/path.md)
  - [文件操作](./java/file.md)
  - [压缩解压](./java/compress/compress.md)
    - [zip压缩](./java/compress/zip_compress.md)
    - [zip解压](./java/compress/zip_extract.md)

- [日期操作](./java/date/date.md)
  - [获取当前日期和时间](./java/date/current.md)
  - [字符串转日期和时间](./java/date/parse.md)
  - [日期和时间转字符串](./java/date/format.md)
  - [初始化日期和时间](./java/date/init.md)
  - [修改日期和时间](./java/date/modify.md)
  - [日期加减](./java/date/date_plus.md)
  - [时间加减](./java/date/time_plus.md)
  - [日期和时间加减](./java/date/datetime_plus.md)
  - [日期和时间比较](./java/date/compare.md)
  - [两个日期和时间的间隔](./java/date/duration.md)
  - [获取月份的第一天和最后一天](./java/date/month.md)
  - [LocalDateTime和Date互转](./java/date/convert.md)

- [字符串操作](./java/str/str.md)
  - [字符串和字节数组互转](./java/str/bytes.md)
  - [数字格式化](./java/str/number_format.md)
  - [字符串和16进制互转](./java/str/hex.md)
  - [区分UTF-8字符串中的ASCII与非ASCII字符](./java/str/urf8.md)

- [HttpClient](./java/http_client/http_client.md)
  - [基本使用](./java/http_client/basic.md)
    - [同步GET请求](./java/http_client/get.md)
    - [异步GET请求](./java/http_client/get_async.md)
    - [POST请求表单](./java/http_client/post_form.md)
    - [POST请求json](./java/http_client/post_json.md)
  - [响应体处理](./java/http_client/body_handlers.md)
  - [获取响应头](./java/http_client/response_headers.md)

- [JAR文件](./jar/jar.md)
  - [MANIFEST.MF文件](./jar/mf.md)

- [线程](./java/thread/thread.md)
  - [start方法](./java/thread/start.md)
  - [线程的实现方式](./java/thread/impl.md)
  - [线程的创建方式](./java/thread/create/create.md)
    - [继承Thread](./java/thread/create/extend.md)
    - [实现Runnable](./java/thread/create/runnable.md)
    - [实现Callable](./java/thread/create/callable.md)
    - [Executors](./java/thread/create/executors.md)
    - [ThreadPoolTaskExecutor](./java/thread/create/thread_pool_task_executor.md)
  - [守护线程](./java/thread/daemon.md)
  - [常用方法](./java/thread/methods.md)
    - [join方法](./java/thread/join.md)
    - [yield方法](./java/thread/yield.md)
    - [interrupt方法](./java/thread/interrupt.md)
    - [wait方法](./java/thread/wait.md)
    - [生产者-消费者模式](./java/thread/pc.md)
  - [线程间通信方式](./java/thread/thread_communicate.md)
    - [synchronized](./java/thread/thread_communicate_synchronized.md)
    - [ReentrantLock](./java/thread/thread_communicate_reentrant_lock.md)
    - [CountDownLatch](./java/thread/thread_communicate_count_down_latch.md)
    - [LockSupport](./java/thread/thread_communicate_lock_support.md)
    - [Socket](./java/thread/thread_communicate_lock_socket.md)
    - [信号量](./java/thread/thread_communicate_semaphore.md)
    - [管道](./java/thread/thread_communicate_pipe.md)
  - [线程的状态](./java/thread/state.md)
  - [线程池](./java/thread/pool/pool.md)
    - [Executor](./java/thread/pool/executor.md)
    - [CallerRunsPolicy策略的坑](./java/thread/pool/caller_runs_policy.md)

- [IO](./java/io/io.md)
  - [虚拟内存](./java/io/virtual_memory.md)
  - [内核空间和用户空间](./java/io/space.md)
  - [数据传输机制](./java/io/transfer.md)
  - [文件描述符](./java/io/fd.md)
  - [IO模型](./java/io/io_model.md)
    - [同步阻塞IO](./java/io/bio.md)
    - [同步非阻塞IO](./java/io/nio.md)
    - [IO多路复用](./java/io/io_multiplexing.md)
    - [实时信号驱动IO](./java/io/rtsig.md)
    - [异步IO](./java/io/aio.md)
  - [Java NIO](./java/io/java_nio.md)
    - [Buffer](./java/io/java_buffer.md)
    - [Channel](./java/io/java_channel.md)
      - [用NIO写入文件](./java/io/java_channel_write_file.md)
      - [用NIO读取文件](./java/io/java_channel_read_file.md)
    - [Selector](./java/io/java_selector.md)
      - [实现一个简单的服务器](./java/io/java_selector_server.md)
      - [实现一个简单的聊天室](./java/io/java_selector_chat.md)
  - [零拷贝](./java/io/zero_copy.md)
    - [mmap+write实现零拷贝](./java/io/mmap_write.md)
    - [sendfile实现零拷贝](./java/io/sendfile.md)
    - [splice实现零拷贝](./java/io/splice.md)

- [JNI](./jni/jni.md)
  - [JNI调用c代码](./jni/jni_with_c.md)
  - [JNI调用c++代码](./jni/jni_with_cpp.md)
  - [JNI数据类型](./jni/jni_types.md)
  - [访问基本数据类型](./jni/jni_primitives.md)
  - [访问字符串](./jni/jni_str.md)
  - [访问基本类型数组](./jni/jni_primitive_array.md)
  - [访问对象的字段](./jni/jni_object_fields.md)
  - [访问类变量](./jni/jni_static_vars.md)
  - [访问对象的方法](./jni/jni_obj_method.md)
  - [访问类的静态方法](./jni/jni_static_method.md)
  - [JNI API](./jni/api.md)
    - [JNI操作字符串](./jni/jni_str_op.md)
    - [JNI操作基本类型的数组](./jni/jni_pa_op.md)
    - [JNI操作对象的字段](./jni/jni_fields_op.md)
    - [JNI操作方法](./jni/jni_method_op.md)

- [字节码操作](./bytecode/bytecode.md)
  - [编译期注解处理器](./bytecode/AbstractProcessor.md)
  - [ASM](./bytecode/asm/asm.md)
    - [访问者模式](./bytecode/asm/visitor.md)
    - [引入ASM依赖](./bytecode/asm/pom.md)
    - [认识Java字节码](./bytecode/asm/class_file/bytecode.md)
      - [class文件](./bytecode/asm/concept/class_file.md)
      - [描述符](./bytecode/asm/class_file/descriptor.md)
      - [访问标志](./bytecode/asm/class_file/access_flags.md)
      - [指令集](./bytecode/asm/class_file/opcodes.md)
    - [一个“增强流程”的标准写法](./bytecode/asm/flow.md)
    - [基本用法](./bytecode/asm/quick_start.md)
    - [核心概念](./bytecode/asm/concept/concept.md)
      - [ClassReader](./bytecode/asm/concept/class_reader.md)
      - [ClassWriter](./bytecode/asm/concept/class_writer.md)
      - [ClassVisitor](./bytecode/asm/concept/class_visitor.md)
      - [MethodVisitor](./bytecode/asm/concept/method_visitor.md)
      - [FieldVisitor和AnnotationVisitor](./bytecode/asm/concept/other_visitor.md)
    - [asm-commons](./bytecode/asm/asm_commons.md)
      - [asm-commons里的工具类](./bytecode/asm/asm_commons_utils.md)
    - [Tree API](./bytecode/asm/tree_api.md)
    - [常见用法](./bytecode/asm/c.md)
      - [读取class内容](./bytecode/asm/read_class.md)
      - [生成class](./bytecode/asm/create_class.md)
      - [读取并修改class](./bytecode/asm/edit_class.md)
      - [复制一个类的字段](./bytecode/asm/copy_fields.md)
      - [将class文件转换为ASM代码](./bytecode/asm/class2asm.md)
      - [ASM搭配Java Agent使用](./bytecode/asm/asm_agent.md)
    - [进阶用法](./bytecode/asm/a.md)
      - [泛型](./bytecode/asm/signature.md)
      - [Label](./bytecode/asm/Label.md)
      - [visitXxxInsn方法](./bytecode/asm/visitInsn.md)
  - [Javassist](./bytecode/javassist/javassist.md)
    - [基本用法](./bytecode/javassist/quick_start.md)
    - [ClassPool](./bytecode/javassist/ClassPool.md)
    - [CtClass](./bytecode/javassist/CtClass.md)
    - [创建class文件](./bytecode/javassist/创建class文件.md)
    - [修改jar包中的class文件](./bytecode/javassist/修改jar包中的class文件.md)
    - [运行时创建class](./bytecode/javassist/运行时创建class.md)
    - [运行时修改class](./bytecode/javassist/运行时修改class.md)
  - [JVMTI](./bytecode/jvmti/jvmti.md)
    - [Java Agent](./bytecode/agent/agent.md)
      - [在类加载的时候修改类](./bytecode/agent/premain.md)
      - [修改已经加载的类](./bytecode/agent/agentmain.md)

- [Maven](./maven/maven.md)
  - [设置国内源](./maven/mirror.md)
  - [设置项目jdk版本](./maven/jdk_version.md)
  - [指定主类](./maven/main_class.md)
  - [导入本地jar包](./maven/local_jar.md)
  - [设置私服](./maven/repositories.md)
  - [Maven内置变量](./maven/maven_vars.md)
  - [多模块下扫描不到子模块mybatis.xml](./maven/maven_scan_mapper.md)

- [Gradle](./gradle/gradle.md)
  - [安装](./gradle/install.md)
  - [Gradle项目结构](./gradle/gradle_project.md)
  - [Gradle Wrapper](./gradle/gradle_wrapper.md)
  - [守护进程](./gradle/gradle_daemon.md)
  - [设置国内源](./gradle/repositories.md)

- [Excel](./excel/excel.md)
  - [Excel的XML格式](./excel/cell_xml_format.md)
  - [EasyExcel](./excel/easyexcel/easyexcel.md)
    - [导出表头合并](./excel/easyexcel/merge_header.md)
  - [POI](./excel/poi/poi.md)
    - [内存溢出问题](./excel/poi/out_of_memory.md)
    - [excel读取](./excel/poi/read_large_excel.md)
    - [excel写入](./excel/poi/write_large_excel.md)
    - [合并单元格](./excel/poi/merge.md)
    - [设置单元格样式](./excel/poi/style.md)
    - [设置某些单元格不可编辑](./excel/poi/cell_readonly.md)

- [JSON](./json/json.md)
  - [Jackson](./json/jackson.md)
    - [首个单词为单个字母时驼峰识别失效](./json/jackson_bug.md)

- [日志](./log/log.md)
  - [slf4j](./log/slf4j.md)
  - [log4j](./log/log4j/log4j.md)
    - [基本使用](./log/log4j/quick_start.md)
    - [配置文件](./log/log4j/config.md)
    - [Appender](./log/log4j/appender.md)
    - [Layouts](./log/log4j/layouts.md)
  - [logback](./log/logback/logback.md)
    - [基本使用](./log/logback/quick_start.md)
    - [配置文件](./log/logback/config.md)
    - [Appender](./log/logback/appender.md)
    - [Logger](./log/logback/logger.md)
    - [Encoder](./log/logback/encoder.md)

- [模版渲染](./template/template.md)
  - [freemarker](./template/freemarker/freemarker.md)
    - [基本使用](./template/freemarker/quick_start.md)

- [JGit](./jgit/jgit.md)
  - [基本使用](./jgit/quick_start.md)
  - [创建本地仓库](./jgit/init.md)
  - [打开git仓库](./jgit/open.md)
  - [添加远程仓库](./jgit/remote.md)
  - [设置ssh认证](./jgit/ssh.md)
  - [git clone](./jgit/clone.md)
  - [git pull](./jgit/pull.md)
  - [git add](./jgit/add.md)
  - [git commit](./jgit/commit.md)
  - [git push](./jgit/push.md)

- [JDBC](./jdbc/jdbc.md)
  - [基本使用](./jdbc/quick_start.md)
  - [查询](./jdbc/select.md)
  - [更新](./jdbc/update.md)
  - [新增后获取自增主键](./jdbc/return_generated_keys.md)
  - [事务](./jdbc/transaction.md)
  - [批量执行](./jdbc/batch.md)
  - [使用HikariCP连接池](./jdbc/pool.md)

- [GraalVM](./graalvm/graalvm.md)
  - [基本使用](./graalvm/quick_start.md)
  - [编译maven项目](./graalvm/mvn.md)
  - [编译SpringBoot3](./graalvm/sb3.md)
