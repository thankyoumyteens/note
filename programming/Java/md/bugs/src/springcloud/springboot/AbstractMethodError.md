...
发现是将实体类转成 json 时报的错, 本地没问题, 部署到服务器上就报错。

本地查找 EventSignStatusChanged.getAgentUUID()方法, 没找到。

把服务器上的 jar 包下载下来查找 EventSignStatusChanged.getAgentUUID()方法, 发现在 EventSignStatusChanged 的父类的父类 Event 类里。

把线上的 Event 类与本地的 Event 类对比, 发现内容不一致。
...
原因: 本地和线上用的不同的 maven 仓库, 下载的 jar 包不一样。
