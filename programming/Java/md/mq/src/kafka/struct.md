...
Segment File：将大文件拆分成小文件来存储, 这样一个大文件就变成了一段一段(Segment)。这样的好处是 IO 加载速度快, 不会有很长的 IO 加载时间。

在 Kafka 的数据文件夹下, 分为两种类型的文件：索引文件(Index File)和日志文件(Log File)。索引文件存的是消息的索引信息, 帮助快速定位到某条消息。日志文件存储的是具体的消息内容。

![](../img/kafka_struct.png)

## 索引文件(Index File)

索引文件的命名统一为数字格式, 其名称表示 Kafka 消息的偏移量。假设索引文件的文件名为 `N.index`, 那么就代表该索引文件存储的第一条 Kafka 消息的偏移量为 `N + 1`, 而上个文件存储的最后一条 Kafka 消息的偏移量为 `N`(因为 Kafka 是顺序存取的)。例如索引文件 `368769.index`, 表示该文件存储的第一条 Kafka 消息的偏移量为 368770。而 368769 表示的是 `0000.index` 这个索引文件的最后一条消息。所以 `368769.index` 存储的 Kafka 消息偏移量范围为 368769 ~ 737337。
...
其中 offset 表示索引文件里的第几条消息, 而物理偏移量则表示该条消息在日志文件中的物理偏移地址。例如 `368769.index` 中的 `3,497` 表示: 索引文件里的第 3 条消息(即 offset 为 368769 + 3 = 368772 的消息), 其在数据文件中的物理偏移地址为 497。

![](../img/kafka_index_struct.png)

## 日志文件(Log File)

日志文件的命名格式与索引文件的命名格式完全一样, 日志文件就是所有消息的一个列表, 而每条消息都有一个固定的格式。
