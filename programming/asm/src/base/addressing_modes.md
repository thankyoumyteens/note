...
```x86asm
# 把内存地址0x123456位置的数据加载到%eax
movl 0x123456, %eax
```

## 索引寻址

使用 `地址或偏移量`, `索引寄存器` 以及 `比例因子` 实现。可以将任何通用寄存器用作 `索引寄存器`。

例如, 有一个名为 `string_start` 的字节串, 并想访问其中第三个字节, `%ecx` 中的值是 2:

```x86asm
movl string_start(, %ecx, 1), %eax
```

该指令从 `string_start` 处开始, 将该地址与 `1 * %ecx` 相加, 并将所得地址中的数据加载到 `%eax` 中。

## 间接寻址

间接寻址方式从寄存器指定的地址加载值。

例如, 此时 `%eax` 保存着一个地址, 可通过以下操作将该地址中的数据移入 `%ebx`:
...
上面的指令会把 `4(%eax)`, `5(%eax)`, `6(%eax)`, `7(%eax)` 4 个字节的数据复制到到 `%ebx` 中。

## 立即寻址

立即寻址与通用格式不同, 立即寻址方式用于将直接值加载到寄存器或存储位置。

```x86asm
# 把数字 0x123456 加载到%eax
movl $0x123456, %eax
```

为了表明立即寻址方式, 必须在数字前加上一个 `$` 符号。否则就会变成直接寻址方式, 把地址为 0x123456 的数据加载到 `%eax`, 而不是数字 0x123456 本身。
