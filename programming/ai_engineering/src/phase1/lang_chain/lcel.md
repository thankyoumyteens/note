...
1. PydanticOutputParser 的魔法：
   - 你不需要自己在 Prompt 里苦口婆心地教大模型怎么写 JSON（比如“请严格输出大括号，不要加 markdown 标记...”）。`parser.get_format_instructions()` 会根据你的 Pydantic 模型自动生成一套完美严苛的指令，塞进 System Prompt 里。
2. `chain = prompt | model | parser` 是如何工作的？在 Python 中，LangChain 重载了 `|` 运算符（通过 `__or__` 魔术方法）。
