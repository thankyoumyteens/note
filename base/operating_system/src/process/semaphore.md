...
若进程 P1 先执行到 V 操作, 则 S=1。之后当执行到 P 操作时, 由于 S=1, 表示有可用资源, P2 进程不会阻塞, 而是继续往下执行。

若先执行到 P 操作, 由于 S=0, 表示此时没有可用资源, 因此 P 操作会阻塞进程 P1。之后当进程 P1 执行完 V 操作, 使 S 变为 1, 由于此时有进程在该信号量对应的阻塞队列中, 因此会在 V 操作中唤醒 P2 进程。这样 P2 就可以继续执行了。
...
信号量机制可以用来表示进程之间的前驱关系, 即一个进程必须等待一个或多个前驱进程完成某个操作后才能开始执行。这种机制通常用于确保进程或任务按照特定的顺序执行, 例如在解决生产者-消费者问题或读者-写者问题时。

在实现前驱关系时, 可以使用有向图来表示进程之间的依赖关系。图中的每个节点代表一个进程, 边表示一个进程必须等待另一个进程完成。信号量可以用来确保在沿着有向图的路径上, 每个进程在开始执行之前, 其所有前驱进程都已经完成。
