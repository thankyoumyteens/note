...
机器周期（Machine Cycle）是计算机操作的基本时间单位，它通常由多个时钟周期组成。一个机器周期可以完成一个基本的操作，如从内存中读取一个指令字或完成一次数据传输。例如，在早期的 8086 微处理器中，一个机器周期用于完成一次内存读写操作，它包含了 4 个时钟周期，每个时钟周期执行不同的微操作。
