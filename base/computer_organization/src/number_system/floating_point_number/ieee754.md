...
- 当阶码为全为 0 且尾数也为全 0 时, 表示的真值为 0, 结合符号位为 0 或 1, 有正零和负零之分
- 当阶码为全为 0 而尾数不全为 0 时, 表示非规格化小数
- 当阶码为全为 1 且尾数也为全 0 时, 表示的真值为无穷大, 结合符号位为 0 或 1, 有正无穷和负无穷之分
- 当阶码为全为 1 而尾数不全为 0 时, 表示非数值(NaN)
