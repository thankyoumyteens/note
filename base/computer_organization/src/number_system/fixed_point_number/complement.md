...
在时钟的表盘上, 一共只有 12 个数字, 所以超出 12 的数字要对 12 取模: 6 + 9 = 15 mod 12 = 3。

可以看到, 6 - 3 和 6 + 9 是等效的,而表盘上 -3 和 +9 的距离正好是 12, 所以要计算 6 - 3, 可以用以下步骤:
...
同理, 4 位二进制 0101 - 0001 的计算方法:

1. 由于是 4 位二进制数, 所以要对 2<sup>4</sup> = 16(二进制 10000) 取模
