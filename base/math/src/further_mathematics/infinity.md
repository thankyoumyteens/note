...
\begin{align}
& \; {\large 无穷小} \\
& 设 f(x) 在 x = a 的去心邻域内有定义, 如果 \lim_{x \to a} f(x) = 0, \\
& 则称: f(x) 在 x \to a 时为无穷小。 \\
\\
& \; 0 是无穷小, 是唯一的一个与自变量趋向无关的无穷小 \\
\\
& 一个非 0 的函数是不是无穷小, 与自变量的趋向有关, \\
...
& \; 1、\alpha \sim \alpha \qquad (\sim 表示等价无穷小) \\
& \; 2、若 \alpha \sim \beta, 则 \beta \sim \alpha \\
& \; 3、若 \alpha \sim \beta, \beta \sim \gamma, 则 \alpha \sim \gamma \\
& \; 4、若 \alpha \sim \alpha_{1}, \beta \sim \beta_{1}, 且 \lim \frac{\beta}{\alpha} = A \\
& \quad \;\; 则: \lim \frac{\beta_{1}}{\alpha_{1}} = A \\
\end{align}
-->
