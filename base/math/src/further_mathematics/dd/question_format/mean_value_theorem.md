...
& {\Large 型1：f^{(n)}(\xi)=0} \\
\\
& {\color{Orange} {\large 一、如果n=1, 则需要找两个相等的函数值 f(a)=f(b)}} \\
\\
...
\\
& {\color{Orange} {\large 二、如果n=2, 则\left\{\begin{matrix}
找三个点用两次拉格朗日定理 \\
找两个点的一阶导数相等，使用罗尔定理
\end{matrix}\right.}} \\
\\
...
& \frac{f(1)+2f(2)}{3} 是介值 \\
& 根据介值定理，存在 x_1 \in [1, 2], 使 f(x_1) = \frac{f(1)+2f(2)}{3} \\
& 得到：3f(x_1) = f(1)+2f(2) \\
& 同理，对f(3)+f(4)+f(5)使用介值定理 \\
& 存在 x_2 \in [3, 5], 使 f(x_2) = \frac{f(3)+f(4)+f(5)}{3} \\
& 得到：3f(x_2) = f(3)+f(4)+f(5) \\
& 因为 f(0)=f(x_1)=f(x_2) \\
& 根据罗尔定理，存在 \xi_1 \in (0, x_1)，\xi_2 \in (x_1, x_2)，\\
& 使 f'(\xi_1) = f'(\xi_2) = 0 \\
...
& {\color{BLue} 证明：存在\xi\in(0, 1)，使f''(\xi)=0} \\
& 因为f(x)二阶可导，则 f(x)连续 \\
& {\color{Green} // 连续, 则函数值等于极限值} \\
& \lim_{x\to0}\frac{f(x)-1}{x}=1，则 f(0)=1 \\
& \lim_{x\to0}\frac{f(x)-1}{x}=\lim_{x\to0}\frac{f(x)-f(0)}{x-0}=1，则 f'(0)=1 \\
& {\color{Green} // f(0)和f(1)两个点不相等，使用拉格朗日中值定理} \\
& 存在c\in(0, 1)，使f'(c)=\frac{f(1)-f(0)}{1-0}=1 \\
& 因为f'(c)=f'(0)=1，且f(x)二阶可导 \\
& {\color{Green} // 使用罗尔定理} \\
& 则存在\xi\in(0, c)，使得f''(\xi)=0 \\
\end{align}
-->
